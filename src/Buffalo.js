    ...
    if (userData) {
      await axios.get(baseUrl + `product/get-product?letter=${letter === "" ? null : letter}`, config).then((res) => {
        ...
        data = res?.data?.product.filter((v) => {
          if (v?.categoryId?._id === "6410928ca55784316868ff9e") {
            return v
          }
        })
        setData(data)
    ...
    <>
      <div style={{ position: 'relative' }}>
        ...
        {/* <div class="input-group search-bar">
          <input type="text" class="form-control" placeholder="Search Buffalo" aria-label="Search Buffalo" aria-describedby="basic-addon2" />
