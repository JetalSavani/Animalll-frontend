...
export default function UpdateProduct() {
    ...
    const getCategory = async () => {
        await axios
            .get(baseUrl + "category/get-category", config)
            .then((res) => {
                console.log("res", res);
                setCategory(res?.data?.category);
                setNewData({ ...newData, c_id: res?.data?.category[0]?._id });
            })
            .catch((err) => {
                console.log("err", err);
            });
    ...
    const getSubCategory = async () => {
        await axios
            .get(baseUrl + "subcategory/get-subcategory", config)
            .then((res) => {
                console.log("res", res);
                setSubCategory(res?.data?.subcategory);
                // setNewData({ ...newData, s_c_id: res?.data?.subcategory[0]?.name })
            })
            .catch((err) => {
                console.log("err", err);
            });
    ...
    const getColor = async () => {
        await axios
            .get(baseUrl + "color/get-color", config)
            .then((res) => {
                console.log("res", res);
                setColor(res?.data?.color);
                // setNewData({ ...newData, s_c_id: res?.data?.color[0]?.name })
            })
            .catch((err) => {
                console.log("err", err);
            });
    ...
    const handleSubmit = async () => {
        ...
        let breed = subCategory.filter(
            (v) => v?.categoryId?._id === dataAnimal[0]?._id
        );
        console.log("breed", breed);
        let body = {
            animalType: dataAnimal[0]?.name,
            categoryId: dataAnimal[0]?._id,
            breedType: newData?.s_c_id ?? breed[0]?.name,
            age: String(newData?.age),
            weight: String(newData?.weight),
            price: newData?.price,
            desc: newData?.description,
            color: newData?.color_id ?? "Black",
            city: newData?.city,
            state: newData?.state,
            ...
            calfAge: String(newData?.calf_age),
            calfGender: gender,
            frontPhoto: image,
            backPhoto: image1,
            generalReport: image2,
            lumpiCertificate: image3,
        };
        console.log("body", body);
        await axios
            .put(baseUrl + `product/update-product?id=${newData?.id}`, body, config)
            .then((res) => {
                console.log("res", res);
    ...
    const handleChangeImage = async (e) => {
        ...
        await axios
            .post(baseUrl + "image/upload-image", formdata, config)
            .then((res) => {
                console.log("res", res);
        ...
        axios
            ...
            .then((res) => {
                ...
                setNewData({
                    ...
                    s_c_id: data?.breedType,
                    age: data?.age,
                    weight: Number(data?.weight),
                    price: data?.price,
                    description: data?.desc,
                    color_id: newData?.color,
                    city: data?.city,
                    state: data?.state,
                    seller_mobile: data?.phone,
                    milk: data?.milk,
                    // isCalf: calf,
                    lactation: data?.lactation,
                    calf_age: Number(data?.calfAge),
                    // calfGender: gender,
                    image: data?.frontPhoto,
                    image1: data?.backPhoto,
                    image2: data?.generalReport,
                    image3: data?.lumpiCertificate,
                });
                // setCalf(data?.isCalf);
                setGender(data?.calfGender);
                setImage(data?.frontPhoto);
                setImage1(data?.backPhoto);
                setImage2(data?.generalReport);
                setImage3(data?.lumpiCertificate);
            })
            .catch((err) => {
                console.log("err", err);
            });
